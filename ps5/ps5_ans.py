...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        ...
        short_word_list = word_list[:]
                
        for i, word in enumerate(word_list):
            if word == phrase_list[0]:
                short_word_list = short_word_list[i:]
                break
        
        if len(short_word_list) < len(phrase_list):
            return False
        
        for j in range(len(phrase_list)):
            if phrase_list[j] != short_word_list[j]:
                return False
        return True
...
# BeforeTrigger pulls the trigger for story pubdate before the input time
...
# AfterTrigger pulls the trigger for story pubdate after the input time
...
# the output of another trigger is true/false
...
def read_trigger_config(filename):
    ...
    for line in lines:
        ...
        if trig [1] == "TITLE":
            ...
            trig_dict[trig[0]] = NotTrigger(trig[2], trig[3])
        elif trig [1] == "OR":
            trig_dict[trig[0]] = OrTrigger(trig[2], trig[3])
        elif trig [0] == "ADD":
            for elm in trig[1:]:
                trig_list.append(trig_dict[elm])
